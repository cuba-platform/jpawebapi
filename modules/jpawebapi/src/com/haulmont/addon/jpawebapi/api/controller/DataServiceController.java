...
import com.haulmont.addon.jpawebapi.api.Authentication;
import com.haulmont.addon.jpawebapi.api.ParseUtils;
import com.haulmont.addon.jpawebapi.api.RestServiceException;
import com.haulmont.addon.jpawebapi.api.RestServicePermissions;
import com.haulmont.addon.jpawebapi.api.config.JpaWebApiConfig;
import com.haulmont.addon.jpawebapi.api.converter.ConversionFactory;
import com.haulmont.addon.jpawebapi.api.converter.Converter;
import com.haulmont.addon.jpawebapi.api.model.CommitRequest;
import com.haulmont.addon.jpawebapi.api.model.QueryRequest;
import com.haulmont.addon.jpawebapi.api.model.ServiceRequest;
...
public class DataServiceController {
    ...
    public void queryByGet(@PathVariable String type,
        ...
        try {
            ...
            ctx.setLoadDynamicAttributes(Boolean.TRUE.equals(dynamicAttributes));
            ...
            if (viewName == null) {
                View view = metadata.getViewRepository().getView(metaClass, View.LOCAL);
                ctx.setView(new View(view, "local-with-system-props", true));
            } else {
                ctx.setView(viewName);
            }

            //noinspection unchecked
            List<Entity> entities = dataService.loadList(ctx);
            Converter converter = conversionFactory.getConverter(type);
            String result = converter.process(entities, metaClass, ctx.getView());
    ...
    protected boolean isNewInstance(Set<String> newInstanceIds, Object instance) {
    ...
    protected void assignUuidToNewInstances(Collection commitInstances, Collection newInstanceIds) {
        for (Object id : newInstanceIds) {
    ...
    public void serviceByGet(@PathVariable(value = "type") String type,
        ...
        if (restServicePermissions.isNotPermitted(serviceName, methodName)) {
            response.sendError(HttpServletResponse.SC_FORBIDDEN);
            return;
        }
        try {
    ...
    public void serviceByPost(@RequestParam(value = "s") String sessionId,
        ...
        try {
            ...
            if (restServicePermissions.isNotPermitted(serviceRequest.getServiceName(), serviceRequest.getMethodName())) {
                response.sendError(HttpServletResponse.SC_FORBIDDEN);
                return;
            }
    ...
    protected void writeResponse(HttpServletResponse response, String result, MimeType mimeType) throws IOException {
    ...
    protected void sendError(HttpServletRequest request, HttpServletResponse response, Throwable e) throws IOException {
        log.error("Error processing request: " + request.getRequestURI() + "?" + request.getQueryString(), e);

        boolean isProductionMode = jpaWebApiConfig.getProductionMode();
    ...
    protected Object parseQueryParameter(String paramKey, String paramValue, Map<String, String[]> queryParams) {
     ...
     */
    protected boolean commitPermitted(Collection commitInstances, Collection newInstanceIds) {
        for (Object commitInstance : commitInstances) {
     ...
     */
    protected boolean removePermitted(Collection removeInstances) {
    ...
    protected boolean createPermitted(MetaClass metaClass) {
    ...
    protected boolean removePermitted(MetaClass metaClass) {
