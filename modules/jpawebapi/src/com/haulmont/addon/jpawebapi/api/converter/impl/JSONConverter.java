...
package com.haulmont.addon.jpawebapi.api.converter.impl;
...
import com.haulmont.addon.jpawebapi.api.converter.ConverterHelper;
import com.haulmont.addon.jpawebapi.api.model.*;
...
public class JSONConverter implements Converter {
    ...
    @Inject
    protected JpaWebApiConfig restConfig;
    ...
    public String process(Entity entity, MetaClass metaclass, View view) {
        MyJSONObject jsonObject = encodeInstance(entity, new HashSet<>(), metaclass, view);
        return jsonObject.toString();
    }

    protected MyJSONObject _process(Entity entity) {
        return encodeInstance(entity, new HashSet<>(), entity.getMetaClass(), null);
    }
    ...
    protected MyJSONObject.Array _process(List<Entity> entities, MetaClass metaClass, View view) {
        MyJSONObject.Array result = new MyJSONObject.Array();
        for (Entity entity : entities) {
            MyJSON item = encodeInstance(entity, new HashSet<>(), metaClass, view);
            result.add(item);
        }
        return result;
    }
    ...
    public String process(Set<Entity> entities) {
        MyJSONObject.Array result = new MyJSONObject.Array();
        for (Entity entity : entities) {
            MyJSONObject entityJson = encodeInstance(entity, new HashSet<>(), getMetaClass(entity), null);

            if (restConfig.getRestApiVersion() == 1) {
                //return map for old version of rest api
                MyJSONObject valueJson = encodeInstance(entity, new HashSet<>(), getMetaClass(entity), null);
    ...
    protected List<Entity> parseIntoList(CommitRequest commitRequest, JSONArray nodeList)
            throws JSONException, InstantiationException, IllegalAccessException, ParseException {
    ...
    protected void asJavaTree(CommitRequest commitRequest, Entity entity, MetaClass metaClass, JSONObject json)
        ...
        while (iter.hasNext()) {
            ...
            checkNotNullArgument(metaPropertyPath,
                    "Could not resolve property '%s' in '%s'", propertyName, metaClass);
            ...
            switch (property.getType()) {
                ...
                case ENUM:
                    if (!json.isNull(propertyName)) {
                        setField(entity,
                                propertyName,
                                property.getRange().asEnumeration().parse(json.getString(propertyName)));
                    } else {
                        setField(entity, propertyName, null);
                    }
                    break;
                ...
                case ASSOCIATION:
                    ...
                    if (!updatePermitted(propertyMetaClass) && readNotPermitted(propertyMetaClass))
                        break;
    ...
    @Override
    public QueryRequest parseQueryRequest(String content) throws IllegalArgumentException {
        JSONObject jsonObject = new JSONObject(content);
    ...
    protected MyJSONObject encodeInstance(final Entity entity, final Set<Entity> visited, MetaClass metaClass, View view) {
        ...
        if (entity instanceof BaseGenericIdEntity) {
            byte[] securityToken = BaseEntityInternalAccess.getSecurityToken(entity);
        ...
        for (MetaProperty property : properties) {
            ...
            switch (property.getType()) {
                ...
                case ASSOCIATION: {
                    ...
                    if (readNotPermitted(meta))
                        break;

                    //noinspection ConstantConditions
                    View propertyView = (view == null || view.getProperty(property.getName()) == null
                            ? null
                            : view.getProperty(property.getName()).getView());
    ...
    protected boolean readNotPermitted(MetaClass metaClass) {
